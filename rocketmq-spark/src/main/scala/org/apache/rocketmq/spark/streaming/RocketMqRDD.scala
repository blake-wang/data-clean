...
class RocketMqRDD (
  ...
  private val cacheInitialCapacity =
    if(optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_INIT_CAPACITY)!=null){
      optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_INIT_CAPACITY).toInt
    }else{
      16
    }
  private val cacheMaxCapacity =
    if(optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_MAX_CAPACITY)!=null){
      optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_MAX_CAPACITY).toInt
    }else{
      64
    }
  private val cacheLoadFactor =
    if(optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_LOAD_FACTOR)!=null){
      optionParams.get(RocketMQConfig.PULL_CONSUMER_CACHE_LOAD_FACTOR).toFloat
    }else{
      0.75F
    }
