...
class MQPullInputDStream(
  ...
  private val maxRateLimitPerPartition =
    if(optionParams.get(RocketMQConfig.MAX_PULL_SPEED_PER_PARTITION)!=null){
      optionParams.get(RocketMQConfig.MAX_PULL_SPEED_PER_PARTITION).toInt
    }else{
      -1
    }
  ...
  private def fetchSubscribeMessageQueues(topics : ju.Collection[jl.String]): ju.HashSet[MessageQueue] = {
    ...
    while (iter.hasNext){
      try {
        ...
        case ex: MQClientException=>
          ex.printStackTrace()
          System.out.println(new Date()+" Error: catch RemotingTimeoutException!"+ex.getCause.getMessage)
      }
