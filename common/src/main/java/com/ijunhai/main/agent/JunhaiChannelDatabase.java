...
public class JunhaiChannelDatabase implements Serializable {
    private static String GROUP = "JunhaiChannelDatabase";
...
//       bjc-apilog.dalan.net:19876 dlUserOrder 60 none hdfs://Ucluster/tmp/ dalan
//       bjc-apilog.dalan.net:19876 dlUserOrder 60 none hdfs://Ucluster/tmp/ dalan YouyunOrder
...
//        optionParams.put(RocketMQConfig.CONSUMER_GROUP,"testOrder");
        ...
        String appName = nameServer + ":JunhaiChannelDatabase";
...
//                    ConsumerStrategy.lastest(), false, false, true, locationStrategy, optionParams);
        ...
        dStream.transform(new Function<JavaRDD<MessageExt>, JavaRDD<MessageExt>>() {
            ...
            public void call(JavaRDD<MessageExt> messageExtJavaRDD) throws Exception {
                ...
                AgentProcess.JunhaiChannelDatabase(messageExtJavaRDD.rdd(), kafkaSinkBroadcast, GPSink, column, finalDesTopic, hdfsCachePath, offsetRanges.get(), nameServer, topics, GROUP, system, redisSinkCluster);
            }
        });
        sc.start();
        sc.awaitTermination();
