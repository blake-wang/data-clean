 ...
 */
public class Mq2KafkaSink implements Serializable {
    private static String GROUP ="Mq2KafkaSink";
...
//        optionParams.put(RocketMQConfig.CONSUMER_GROUP,"testOrder");
        ...
        String appName=nameServer+":Mq2KafkaSink";
...
//        assert redisConn != null;
        ...
        dStream.transform(new Function<JavaRDD<MessageExt>, JavaRDD<MessageExt>>() {
            ...
            public void call(JavaRDD<MessageExt> messageExtJavaRDD) throws Exception {
                ...
                AgentProcess.processSink(messageExtJavaRDD.rdd(), kafkaSinkBroadcast,offsetRanges.get(),nameServer,topics, GROUP,redisSinkCluster);
            }
        });
        sc.start();
        sc.awaitTermination();
