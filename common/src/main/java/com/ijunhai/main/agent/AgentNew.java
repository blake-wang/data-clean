...
public class AgentNew implements Serializable {
    private static String GROUP = "AgentNew";
    public static void main(String[] args) throws InterruptedException {
        ...
        String appName = nameServer + ":AgentNew";
        ...
        if (!queueToOffset.isEmpty()) {
            ...
            dStream = RocketMqUtils.createJavaMQPullStream(sc, GROUP, topics,
                    ConsumerStrategy.earliest(), false, false, true, locationStrategy, optionParams); //线上环境需要改成earlist
        }
        ...
        dStream.transform(new Function<JavaRDD<MessageExt>, JavaRDD<MessageExt>>() {
            ...
            public void call(JavaRDD<MessageExt> messageExtJavaRDD) throws Exception {
                if (Save2Redis.isRunning(appName, uuid, Integer.parseInt(second), redisSink)) {
                    System.out.println(appName + " is running and the uuid is " + uuid);
                    System.exit(1);
                }
                AgentProcess.agentNewProcess(messageExtJavaRDD.rdd(), kafkaSinkBroadcast, GPSink, offsetRanges.get(), nameServer, topics, GROUP, redisSink);
            }
        });
