 ...
 */
public class DB2GP implements Serializable {
    public static String GROUP = "DB2GP";
...
//        optionParams.put(RocketMQConfig.CONSUMER_GROUP,"testOrder");
        ...
        String appName = nameServer + ":DB2GP";
...
//        assert redisConn != null;
        ...
        dStream.transform(new Function<JavaRDD<MessageExt>, JavaRDD<MessageExt>>() {
            ...
            public void call(JavaRDD<MessageExt> messageExtJavaRDD) throws Exception {
                ...
                DB2GPProcess.process(messageExtJavaRDD.rdd(), kafkaSinkBroadcast, GreenPlum, finalDesTopic, offsetRanges.get(), nameServer, topics, GROUP, redisSinkCluster);
            }
        });
        sc.start();
        sc.awaitTermination();
