...
//    System.setProperty("hadoop.home.dir", "E:\\64bit_software\\64bit_software\\hadoop-2.6.0-x64\\hadoop-2.6.0")
    ...
    if(args.length<5){
      System.err.println("<SERVER:PORT> <TOPIC1,TOPIC2,...> <SECOND> <HDFS_CACHE_PATH> <WRITE_TOPIC>")
      System.exit(1)
    }
    ...
    val sparkConf=new SparkConf().setAppName("OrderJoin")
    //      .setMaster("local")
...
//    })
    ...
    KafkaLogProcess.orderJoinTest(kafkaStream.map(_.value()),kafkaSinkBroadcast, "channel_id", finalDesTopic, hdfsCachePath,redisSinkCluster)
