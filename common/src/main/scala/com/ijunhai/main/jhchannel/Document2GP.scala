...
import com.ijunhai.process.jhchannel.{Doc2GPProcess, Log2GPProcess}
...
object Document2GP {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val appName = topicsStr + ":Document2GP"
    ...
    val bGreenPlumSimk: Broadcast[GreenPlumSink] = ssc.sparkContext.broadcast(GreenPlumSink.apply(if (destTopic.equals(Log2GPProcess.DALAN)) Log2GPProcess.DALAN_DATABASE else Log2GPProcess.CHUMENG_DATABASE))
    brokerList = server
    Save2Kafka.setBroker(brokerList)
    val group = "Document2GP"
    ...
    kafkaStream.map(_.value()).foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        //        println(rdd.count())
        Doc2GPProcess.process(rdd, kafkaSinkBroadcast, bGreenPlumSimk, destTopic)
      }
    })
