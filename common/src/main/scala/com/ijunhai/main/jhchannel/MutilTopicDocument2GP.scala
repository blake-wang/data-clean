...
object MutilTopicDocument2GP {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val appName=topicsStr+":"+this.getClass.getName
    ...
    var lastOffset=Array[OffsetRange]()
    brokerList = server
...
//    })
    ...
    kafkaStream.map(_.value()).foreachRDD(rdd=>{
      if(!rdd.isEmpty()){
          if(rdd.collect().contains("asd"))
            args(4)

//        println(rdd.count())
//        Document2GpProcess.process(rdd,kafkaSinkBroadcast,destTopic)
      }
    })
    ...
    try {
      ...
      case e:Exception=>
        ...
        if(!lastOffset.isEmpty){
          topics.foreach(topic=>{
            redisKafkaOffset.saveOffset(lastOffset, brokerList, topic, group,redisSinkCluster)
          })
        }
    }
  }
